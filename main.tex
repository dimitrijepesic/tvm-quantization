...
In a separate experiment we tested the VNNI-preferred configuration directly (\texttt{dtype\_input="uint8"}, \texttt{dtype\_weight="int8"}). In that session the uint8/int8 configuration measured 330.19\,ms versus 365.64\,ms for signed int8/int8 -- marginally faster (${\sim}10\%$), but still far slower than the fp32 baseline (${\sim}88$\,ms, measured in the earlier session under the same hardware). This is consistent with the absence of autotuning being the dominant factor on x86: even on the dispatch-preferred path, untuned schedules cannot beat the simpler fp32 graph.
