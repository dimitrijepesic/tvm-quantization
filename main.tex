...
\paragraph{Logical sub-byte mode.} The split between quantization logic and downstream dtype handling can be confirmed directly. If we set \texttt{nbit\_input=N} (with $N \in \{1,2,4\}$) but keep \texttt{dtype\_input="int8"} and \texttt{dtype\_weight="int8"} -- a configuration we call \emph{logical} sub-byte -- the quantization pass reduces the effective range to $N$ bits while keeping the IR's storage dtype at int8. Build and execution then succeed at every value of $N$ because no sub-byte dtype ever flows downstream; the same int8 kernels run, and the measured latencies match the int8 baseline within noise (the kernels are not aware that fewer bits of range are actually in use). This is not a path to a faster model -- no kernel exploits the narrower range -- but it isolates the diagnosis: the quantization pass itself has no objection to $\mathrm{nbit} < 8$, and what failed at native int4/int2/int1 is downstream dtype handling, not the quantization logic. The logical-mode measurements are produced by \texttt{experiments/02\_bench\_varied\_bitwidth.py}.
